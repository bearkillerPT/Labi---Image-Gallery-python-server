...
getList = function() {
    ...
    xhttp.open("GET", "https://xcoa.av.it.pt/labi2019-p2-g2/list?type=names", false);
    xhttp.send(null);
...
getImagesByName = function(self, divname) {
    ...
    if (checkBoxStatus && colorinfo) {
        ...
        request = "https://xcoa.av.it.pt/labi2019-p2-g2/list?type=detected&name=" + name + "&color=" + color + "&page=1&per_page=10&thr=" + thr;
    } else {
        request = "https://xcoa.av.it.pt/labi2019-p2-g2/list?type=detected&name=" + name + "&page=1&per_page=10&thr=" + thr;
    }
    var div;
    if (divname) {
    ...
    if (name !== "") {
        for (var i = 0; i < res[name].length; i++) {
            imageArray += "<div class=\"grid-item " + name + "\"><img src='https://xcoa.av.it.pt/labi2019-p2-g2/images/" + res[name][i].image + "' width=\"" + Math.round($(window).width() / 4.2) + "\"/></div>";
        }
    } else {
        for (var i = 0; i < res.length; i++) {
            imageArray += "<div class=\"grid-item \"><img src='https://xcoa.av.it.pt/labi2019-p2-g2/images/" + res[i].image + "' width=\"" + Math.round($(window).width() / 4.2) + "\"/></div>";
        }
    }

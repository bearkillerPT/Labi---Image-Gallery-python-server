...
createDropdown = function() {
  ...
  for (var category of listOfClasses) {
    dropdownin +=
      "<input class='animate' type='radio' name='question' id='q" +
      i +
      "'/><label class='animate' for='q1'>" +
      category +
      '</label><p class=\'response animate\'><div class="grid" id="grid' +
      category +
      '">The images are being loaded! Please wait for the server to respond!</div>';
    i += 1;
  }
...
getImagesByName = function(self, divname) {
  ...
  if (checkBoxStatus && colorinfo) {
    var color =
      ...
      "}";
    request =
      "https://xcoa.av.it.pt/labi2019-p2-g2/list?type=detected&name=" +
      name +
      "&color=" +
      color +
      "&page=1&per_page=10&thr=" +
      thr;
  } else {
    request =
      "https://xcoa.av.it.pt/labi2019-p2-g2/list?type=detected&name=" +
  ...
  if (name !== "") {
    for (var i = 0; i < res[name].length; i++) {
      imageArray +=
        '<div class="grid-item ' +
        name +
        "\"><img src='https://xcoa.av.it.pt/labi2019-p2-g2/images/" +
        res[name][i].image +
        "' width=\"" +
        Math.round($(window).width() / 4.2) +
        '"/></div>';
    }
  } else {
    for (var i = 0; i < res.length; i++) {
      imageArray +=
        '<div class="grid-item "><img src=\'https://xcoa.av.it.pt/labi2019-p2-g2/images/' +
        res[i].image +
        "' width=\"" +
        Math.round($(window).width() / 4.2) +
        '"/></div>';
...
defColor = function(colorArr) {
  ...
  if (name != nameAfter && nameAfter != "") name = nameAfter;
...
sliderobj = function(self) {
  if (self) thr = self.value * 0.01;
};

slidertol = function(self) {
  if (self) tol = self.value * 0.01;
};
